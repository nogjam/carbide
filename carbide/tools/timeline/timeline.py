"""For generating timelines from plain text.

Based on the work at: https://github.com/IBM/timeline-generator
"""
...
def get_timeline(
    ...
    if not end:
        end_datetime = max(
            max(data.start_datetime), max(data.end_datetime)
    ...
    ax.vlines(
        vlines.start_datetime, 0, vlines.height, color=vlines.color, linewidth=0.5
    )
...
def annotate(ax, row):
    description = "\n".join(textwrap.wrap(row.description, width=row["text_wrap"]))
